...
function isSafeRedirect(path: string) {
  // Must be a same-origin relative path: "//evil.com" and "/\evil.com" are protocol-relative
  // or browser-normalized into an external origin despite starting with a single "/".
...
const LoginContent = () => {
  ...
  useEffect(() => {
    if (!isLoading && user) {
      router.replace(isSafeRedirect(next) ? next : "/dashboard");
    }
