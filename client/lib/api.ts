...
export async function apiFetch<T>(
  ...
  try {
    ...
    // A caller-initiated abort (e.g. a component unmounting) should propagate as-is so
    // existing `err.name === "AbortError"` handling keeps working; our own timeout firing,
    // or a genuine network failure (offline, DNS, CORS, backend down), becomes a clean message
    // instead of the browser's raw "Failed to fetch" / "Load failed" text.
